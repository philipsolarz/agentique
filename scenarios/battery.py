...
Real runs through the same harness, breadth as coverage: a plain completion, a
single tool call, multiple tool calls, tool-error recovery, and a bounded loop that
hits max_turns. Each produces its own captured run + digest; the anomalies are then
collected into one cross-scenario summary.

    uv run --extra anthropic python -m scenarios.battery

TODO (Phase 12): the delegation scenario was dropped in the topology realignment.
Delegation now goes through the core Scheduler, so its trace should be captured via
the Scheduler's event stream rather than the seam-wrapping Recorder used here —
re-introduce it as a Scheduler-driven scenario subscribing to that stream.
"""
