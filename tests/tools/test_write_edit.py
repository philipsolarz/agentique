...
async def test_write_creates_file_and_parents(tmp_path: Path) -> None:
    ...
    result = await tool(
        RunContext.for_test(), {"path": "game/snake.html", "content": "<html></html>"}
    )
...
async def test_write_overwrites(tmp_path: Path) -> None:
    tool = WriteFile(Workspace(tmp_path))
    await tool(RunContext.for_test(), {"path": "f.txt", "content": "first"})
    await tool(RunContext.for_test(), {"path": "f.txt", "content": "second"})
...
async def test_write_escape_is_error(tmp_path: Path) -> None:
    ...
    result = await tool(
        RunContext.for_test(), {"path": "../escape.txt", "content": "x"}
    )
...
async def test_write_non_string_content_is_error(tmp_path: Path) -> None:
    tool = WriteFile(Workspace(tmp_path))
    result = await tool(RunContext.for_test(), {"path": "f.txt", "content": 123})
    assert result.is_error is True
...
async def test_edit_replaces_unique_match(tmp_path: Path) -> None:
    ...
    result = await tool(
        RunContext.for_test(), {"path": "f.txt", "old": "world", "new": "snake"}
    )
    assert result.is_error is False
...
async def test_edit_no_match_is_error(tmp_path: Path) -> None:
    ...
    tool = EditFile(Workspace(tmp_path))
    result = await tool(
        RunContext.for_test(), {"path": "f.txt", "old": "absent", "new": "x"}
    )
    assert result.is_error is True
...
async def test_edit_ambiguous_match_is_error(tmp_path: Path) -> None:
    ...
    tool = EditFile(Workspace(tmp_path))
    result = await tool(
        RunContext.for_test(), {"path": "f.txt", "old": "a", "new": "b"}
    )
    assert result.is_error is True
...
async def test_edit_missing_file_is_error(tmp_path: Path) -> None:
    tool = EditFile(Workspace(tmp_path))
    result = await tool(
        RunContext.for_test(), {"path": "nope.txt", "old": "a", "new": "b"}
    )
    assert result.is_error is True
