...
from agentique import Artifact, Store, TextPayload
...
async def test_artifact_round_trips_and_lists() -> None:
    store = Store()
    artifact = Artifact(
        id="a1", kind="plan", payload=TextPayload(text="hello"), status="proposed"
    )
...
async def test_session_record_round_trips_and_lists() -> None:
    store = Store()
    record = SessionRecord(id="s1", agent_id="planner", state="paused", question="ok?")
...
async def test_put_artifact_overwrites_without_duplicating_index() -> None:
    store = Store()
    artifact = Artifact(id="a1", kind="plan", payload=TextPayload(text="v1"))
    await store.put_artifact(artifact)
